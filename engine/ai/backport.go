...
func ConnectAI(game *vii.Game, seat *vii.GameSeat) *AI {
...
func (ai *AI) send(name string, json js.Object) {
	...
	if name == "start" {
		...
		ai.Game.Log().Add("GameId", ai.Game.Key).Add("Username", ai.Seat.Username).Add("EventName", name).Warn("ai: event not recognized")
	}
}
