...
func ConnectAI(game *vii.Game, seat *vii.GameSeat) {
...
func (ai *AI) WriteJson(json js.Object) {
	...
	switch v := data.(type) {
	case js.Object:
		go ai.send(json.Sval("event"), v)
		return
	default:
		ai.Game.Log().Add("data", v).Warn("ai: i don't understand")
	}
}
...
func (ai *AI) send(name string, json js.Object) {
	...
	if name == "/game" {
		ai.EventGame(json)
	} else if name == "/game/event" {
		if event := json.Sval("event"); event == "start" {
			ai.GameEventStart(json)
		} else if event == "main" {
			ai.EventMain(json)
		} else {
		...
		ai.Game.Log().
			Add("GameId", ai.Game.Key).
			Add("Username", ai.Seat.Username).
			Add("EventName", name).
...
func (ai *AI) GameEventStart(data js.Object) {
