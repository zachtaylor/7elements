...
func LoginHandler(rt *runtime.T) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if session, _ := rt.Sessions.Cookie(r); session != nil {
			session.WriteCookie(w, !rt.Settings.Devenv)
		...
		} else if password := internal.HashPassword(r.FormValue("password"), rt.Settings.Salt); password != account.Password {
			http.Redirect(w, r, "/login?password", http.StatusSeeOther)
			log.Warn("wrong password")
		} else if player, err := rt.Login(account); player == nil {
