...
func (texts CardTextService) LoadLanguage(lang string) error {
	rows, err := conn.Query("SELECT cardid, language, name, description, flavor FROM cards_text WHERE language=?",
		lang,
	)
	if err != nil {
		return err
...
func (texts CardTextService) loadPowersCache(lang string) error {
	rows, err := conn.Query("SELECT cardid, powerid, description FROM cards_powers_texts WHERE language=?",
		lang,
	)
	if err != nil {
		return err
