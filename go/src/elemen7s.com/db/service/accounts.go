...
func (_ AccountService) UpdatePacks(account *vii.Account) error {
	_, err := db.Connection.Exec("UPDATE accounts SET packs=? WHERE username=?", account.Packs, account.Username)
	return err
}

func (_ AccountService) UpdateCoins(account *vii.Account) error {
	_, err := db.Connection.Exec("UPDATE accounts SET coins=? WHERE username=?", account.Coins, account.Username)
	return err
}
