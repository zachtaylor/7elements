...
func Load(username string) (Decks, error) {
	rows, err := db.Connection.Query("SELECT username, name, id, wins, register, color FROM accounts_decks WHERE username=?",
		username,
	)
	...
	for rows.Next() {
		...
		err = rows.Scan(&deck.Username, &deck.Name, &deck.Id, &deck.Wins, &registerbuff, &deck.Color)
		if err != nil {
			return nil, err
		}
...
func Insert(username string, deckid int) error {
	...
	for deckid, deck := range insertdecks {
		_, err := db.Connection.Exec("INSERT INTO accounts_decks (username, name, id, wins, register, color) VALUES (?, ?, ?, ?, ?, ?)",
			username,
			deck.Name,
			deck.Id,
			deck.Wins,
...
func UpdateAddWin(username string, id int) error {
	if decks := Test(username); decks == nil {
		return errors.New("decks.UpdateWins: no decks for username")
	} else if deck := decks[id]; deck == nil {
		return errors.New("decks.UpdateWins: deck number invalid")
	} else {
		deck.Wins++
		_, err := db.Connection.Exec("UPDATE accounts SET wins=? WHERE username=? AND id=?", deck.Wins, username, deck.Id)
		return err
	}
}
